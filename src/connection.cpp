...
void Connection::accept()
{
	std::lock_guard<std::recursive_mutex> lockClass(connectionLock);
	try {
		readTimer.expires_from_now(boost::posix_time::seconds(static_cast<int64_t>(Connection::read_timeout)));
...
void Connection::internalSend(const OutputMessage_ptr& msg)
{
	...
	try {
		writeTimer.expires_from_now(boost::posix_time::seconds(static_cast<int64_t>(Connection::write_timeout)));
